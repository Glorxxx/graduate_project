...
def begging_by_tree3(S,T,Ys,n,m,Yt):
    ...
    for i in range(n):
        train_data,test_data=rand_train(S,m,T)
        TAS,TPD=align(train_data,test_data)
        #分别是是标签有多少类，正则化系数，epsilon，一共是10个模型，0.5/10
        all_theta=multi_class.one_vs_all(TAS,Ys,5,1,0.05)
...
def begging_by_tree4(S,T,Ys,n,m,epsilon,lamda):
    ...
    for i in range(n):
        ...
        all_theta=multi_class.one_vs_all_q(TAS,Ys,5,lamda,epsilon_1,q_list[i])
        y_predict=multi_class.predict_all(TPD,all_theta)
        y_predict=np.array(y_predict.ravel())
        y_predict=y_predict.reshape(-1,)
        predict_list.append(y_predict)
...
if __name__ == '__main__':
    src, tar = 'data/VLSC/ImageNet','data/VLSC/Caltech101',
    src_domain, tar_domain = scipy.io.loadmat(src), scipy.io.loadmat(tar)
    S,Ys = src_domain['data'][:,0:4096],src_domain['data'][:,4096]
    T, Yt = tar_domain['data'][:,0:4096], tar_domain['data'][:,4096]
    ...
    # 引用multi_class
    predict_list = begging_by_tree3(S, T, Ys, 10, 1000, Yt)
    predict_label = calc_error(predict_list)
    predict_label = tuple(predict_label)
    acc8 = sklearn.metrics.accuracy_score(Yt, predict_label[0])
    print(acc8)

    predict_list = begging_by_tree4(S, T, Ys, 10, 1000, 0.05,1)
    predict_label = calc_error(predict_list)
    predict_label = tuple(predict_label)
